...
// Fake data
const FAKE_PROJECT = { id: '1', name: 'Demo Project', description: 'A demo project', administrator: true };
const FAKE_MEMBERS = [
...
export default function ProjectSettingsPage() {
  ...
  const [name, setName] = useState(FAKE_PROJECT.name);
  const [description, setDescription] = useState(FAKE_PROJECT.description);
  const [error, setError] = useState('');
  const [members, setMembers] = useState(FAKE_MEMBERS);
  const [datasources, setDatasources] = useState(FAKE_DATASOURCES);
  const [deleteConfirm, setDeleteConfirm] = useState('');

  // Handlers (to be replaced by API calls)
  const handleSave = () => {
